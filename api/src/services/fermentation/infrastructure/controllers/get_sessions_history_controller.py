...
async def get_sessions_history(user_id: int, role: str) -> list[FermentationSessionResponse]:
    repo = FermentationRepository(AsyncSessionLocal)
    sessions = await GetSessionsHistoryUseCase(repo).execute(user_id, role)
