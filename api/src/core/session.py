...
async def rotate_and_revoke(repo, user_id: int) -> str:
    ...
    El paso 2 se hace por dos vías durante la migración del WebSocket a Node:
    - In-process (session_ws_manager): mientras el front siga conectado a /ws/session de este back.
    - RabbitMQ (ws.events): para que el ws-service (Node) lo entregue cuando el gateway ya apunte a él.
    Cuando el WebSocket viva 100% en Node, se elimina la vía in-process.
    ...
    await to_users("session", [user_id], {"type": "session:revoked"})
    return sid
